...
namespace Recipes.Application.UnitTests
{
    public class RecipesServiceTests
    {
        ...
        [InlineData(RecipesSelectionType.Own)]
        [InlineData(RecipesSelectionType.Starred)]
        public async Task GetRecipesPage_NoRecipesSecondPage_ThrowsElementNotFoundException(
            RecipesSelectionType selectionType)
        {
            // Arrange
            _recipesRepoMock.Setup(x =>
            ...
            // Act
            Func<Task<RecipesPageResult>> act = () =>
                _recipesService.GetRecipesPage(1, 2, selectionType, _mainUserClaims);
        ...
        public async Task CreateRecipe_ValidInputData_RecipeIsCreated()
        {
            ...
            // Assert
            _tagsServiceMock.Verify(x => x.GetOrCreateTags(TestRecipeDataProvider.RecipeCreateCommand.Tags),
                Times.Once);
            _recipesRepoMock.Verify(
                x => x.AddRecipe(It.Is<Recipe>(recipe => recipe.AuthorId == expectedRecipeModel.AuthorId)), Times.Once);
        ...
        public async Task DeleteRecipe_NonExistingRecipe_ThrowsElementNotFoundException()
        {
            ...
            await Assert.ThrowsAsync<ElementNotFoundException>(() =>
                _recipesService.DeleteRecipe(recipeId, _mainUserClaims));
            _recipesRepoMock.Verify(x => x.GetById(recipeId), Times.Once);
        }
